...
Now for the axioms: their are pipeline transferring these meanings
p ->(q ->p) 
here q is independent of p
noiselessly.   

LLM  realy predict on meaning, and logic is precisely predicting GAPS without meaning (in abstraction from meaning) but using the meaning of the rule
...
think also about Shannon argument from Joint AEP 
in transmission you can increase the rate of transmission by increasing the block size (redundancy)
you can reach proof conclusion even is you are in a negligeble distance from it.
...
Quine, Załudowski, and Transformees 

Quine forsaw LLMs
Zabudowski forsaw their problems.

My quateion 
The are two main theories of language meaning (truth-condiutioanl and distributional semantics)
Do they conradict each other? Can the be merged?  
can truth-conditions theory of meaning be merged with distributional semantics?

Iw ill argue, that not only they are 
they are complementary and one needs another to exist


TC theory is "deflaionary" without DS
DS is not dournded without TC

Corpiuses  mosty comprise TRUTHs
So synonymy relations (abandoned by Quine) can be established (closeness in hdspace)
But synonymi is consequence of words locations (source of inforamtion )
Words as "information sources" REPLACING objects (represnnt ) - spatiotemporally, yes
So language can match world
So LLMs are grounded 
And therefore truth-condtional theory can be applied co do zasady


Warto byłoby wytrenowac model "Quine'" gavagai na corpusie obciętym do zdań obserwacyjncyh
może z jakiego eksperymtntu

Niedźwiedź goni zająca
